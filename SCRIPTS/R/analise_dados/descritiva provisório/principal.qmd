---
title: "análise descritiva RAMT provisória"
format:
  html: default
---
...
## 1.Introdução
O presente trabalho teve como objetivo a digitalização, uniformização e disponibilização dos dados publicados nos Relatórios de Análises do Mercado de Terras, que são instrumentos de análise dos diferentes mercados fundiários estabelecidos no país, realizados pelas superintendências do INCRA — Instituto Nacional de Colonização e Reforma Agrária — em cada estado (BRASIL, 2014). O material que se buscou digitalizar foram as PPRs — Planilhas de Preços Referenciais de Terras — disponibilizadas nesses relatórios. Essas planilhas possuem os valores de referência utilizados pelo órgão com a finalidade de avaliar imóveis rurais em todo o país (BRASIL, 2014).

Os relatórios de Análise do Mercado de Terras sintetizam informações sobre a terra; nesse sentido, ocorre a separação dos diferentes estados em várias Zonas Homogêneas para fins de análise. Essas Zonas Homogêneas são construídas por meio de um processo de clusterização (BRASIL, 2014). A clusterização ocorre em duas etapas: primeiro, é aplicado o algoritmo de Ward para a definição do número de clusters; posteriormente, por meio de um algoritmo de K-means, são feitos ajustes nas cidades de modo a adequar melhor os clusters. As variáveis recomendadas pelas superintendências do INCRA para serem levadas em conta durante o processo de formação dos agrupamentos (Zonas Homogêneas) são as seguintes (BRASIL, 2014):
```{r}
#| message: false
#| warning: false
library(tidyverse)
...
Após esses processos, são formadas as Zonas Homogêneas, ou mercados de terra, sob os quais são realizadas as análises do INCRA, com a finalidade de determinar os valores da terra na região (BRASIL, 2014). O principal componente das PPRs é a presença de um valor médio para Terra Nua e um valor médio para terra com imóvel, variando de acordo com a tipologia indicada.

Assim, o presente banco de dados é produto da digitalização dos materiais presentes nos diferentes Relatórios de Análise do Mercado de Terras. Foram digitalizados cerca de 133 documentos em PDF, cada um contendo algumas PPRs, havendo sobreposição entre os documentos sintéticos de PPRs e os Relatórios de Análise de Terras. Ademais, foram utilizados dados do Atlas do Mercado de Terrras, uma iniciativa do ano de 2023 voltada à unificação dos preços de terra em um único ano de referência.

Nesse sentido, como as observações são realizadas por diferentes unidades do INCRA em momentos distintos, existe um elevado grau de segregação espaço-temporal dos dados. Além disso, alguns municípios estão faltantes na base, seja por erro de transcrição, seja por ausência de dados nos documentos originais. Nesse sentido, segue a disponibilidade de dados por ano, considerando os territórios municipais:
...
## 2.Metodologia 
A extração das planilhas foi feita com o auxílio da inteligência artificial Google Gemini, por meio de prints das tabelas; as tabelas eram capturadas, transcritas pelo Gemini e, posteriormente, verificadas manualmente. Além disso, alguns scripts para a extração das tabelas foram criados e utilizados, empregando o Camelot no Python ou até mesmo o Shiny no R. Entretanto, devido à diversidade na forma de publicação das tabelas — algumas em PDF, outras em imagem e, por vezes, prints de tabelas com notável dificuldade de legibilidade — a maioria das extrações foi realizada via Gemini.

Acerca da incompatibilidade de dados entre os Relatórios de Análise dos Mercados de Terras (RAMT) e os dados do Atlas para casos de mesmo ano, preferiu-se priorizar os dados presentes nos RAMTs, tendo em vista sua maior frequência. Nesse sentido, os dados de 2023 são majoritariamente provenientes do Atlas, existindo a exceção para o estado do Mato Grosso, pois havia um relatório para o ano de 2023 cujos valores divergiam dos apresentados no Atlas, enquanto os valores apresentados no Atlas foram utilizados para um relatório de 2024. Assim, utilizaram-se os valores do relatório para 2023.

Após a digitalização, os dados foram revisados manualmente. Além disso, buscou-se parear as cidades que anteriormente foram digitalizadas por meio de uma lista que pode ser encontrada na coluna "cluster" dos dados. Para isso, criou-se inicialmente um script de separação da string. As strings foram separadas segundo o seguinte trecho:
...
Após o procedimento de separação, passou-se para a etapa de correspondência (matching) das cidades com o código do IBGE, processo que ocorreu em duas fases. Primeiro, foi realizada uma limpeza nos nomes dos municípios, tanto no dataframe do IBGE quanto no dataframe construído; em seguida, procedeu-se ao matching utilizando a função left_join do pacote dplyr. Inicialmente, esse procedimento deixou algumas cidades sem correspondência. Ademais, foi necessário realizar a correspondência entre o código do IBGE, o número da UF e a sigla da unidade federativa, a fim de viabilizar o procedimento de left_join.
...
Os erros de join foram isolados em um dataframe distinto para correção. A fim de facilitar o processo, utilizou-se uma técnica de fuzzy matching, baseada no cálculo da distância de edição entre as strings do banco de dados original (denominado eco1) e as do IBGE. Estabeleceu-se um limiar de distância de 0,2 para a substituição automática; contudo, mesmo após esse procedimento, algumas inconsistências remanescentes exigiram correção manual.
...
As correções manuais necessárias apresentaram naturezas distintas, decorrentes de uma série de inconsistências. Algumas foram provenientes de limitações na métrica de distância de edição utilizada no processo de fuzzy matching. Outras derivaram da estrutura de construção do dataframe, a exemplo da associação de cidades da Zona Homogênea do Distrito Federal ao código do IBGE da unidade federativa correspondente. Identificaram-se, ainda, nomes incompletos nos próprios Relatórios de Análise do Mercado de Terras e casos de cidades cujos nomes continham a conjunção “e”, as quais haviam sido indevidamente fragmentadas no script de separação inicial, exigindo correções diretamente no código de separação. Ademais, algumas cidades tiveram que ter seus nomes alterados para corresponder aos nomes presentes na tabela do IBGE, tendo em vista que os dados variam de 2015 a 2025. Por fim, observaram-se cidades fronteiriças que, ocasionalmente, constavam em mercados regionais de outros estados.
...
Repetiu-se, então, o procedimento de fuzzy join e, além disso, realizou-se uma verificação acerca dos municípios que se encontravam sem pareamento no join. Ao final do processo, nenhum município da base ficou sem identificação.
```{r}
#| message: false
#| warning: false
#| results: asis 
...
Após esses procedimentos, foi realizada a correção inflacionária pelo índice IGP-DI, que é o índice recomendado pelo INCRA para a correção dos valores da terra, segundo a norma de execução para a elaboração dos relatórios (BRASIL, 2014). A correção inflacionária foi feita com o auxílio da API do IPEA para adquirir as séries temporais de variação do índice.
...
Para além desse dataframe principal (EcoHub), que consolida os dados de todos os relatórios da RAMT, realizou-se um corte transversal referente ao ano de 2023, visando obter a maior completude de dados possível para um mesmo período. Para a elaboração desse segundo banco de dados, aplicaram-se os mesmos procedimentos anteriores, com o diferencial da atualização dos valores monetários para 2023. Além disso, integraram-se dados de municípios que não possuíam observações naquele ano específico; nesses casos, selecionaram-se os registros mais recentes de cada região. Quando ocorria empate na distância temporal, adotou-se o critério de proximidade cronológica sob unidade de tempo anual.
...
Segue a listagem de relatórios que serviram para construção do banco de dados: 
...
Observação: as Zonas Homogêneas do Distrito Federal contemplam uma série de municípios que não pertencem ao estado de Goiás. Nesse sentido, os documentos do Distrito apresentam também municípios de Minas Gerais e da Bahia. As correções da geolocalização desses municípios podem ser vistas na seção de metodologia.


## 4. Análise Descritiva
Para a finalidade da análise descritiva do banco de dados, optou-se por realizar um recorte temporal para o ano de 2023, uma vez que é o ano com a maior disponibilidade de dados contemporâneos para o maior número de municípios ao mesmo tempo. Desse modo, com a finalidade de complementar as informações das cidades para as quais o recorte temporal de 2023 carecia de dados, utilizaram-se observações de outros anos, corrigindo-as por meio do índice IGP-DI, como citado acima.
```{r}
#| message: false
#| warning: false
...
Uma hipótese que se pode formular acerca do número de observações é a de que poderia haver um número maior de observações onde existe maior área; entretanto, isso não se verifica na amostra, como pode ser visto por meio dos scatter plots:
...
As linhas tracejadas representam regressões lineares no modelo log-log, utilizadas para verificar a elasticidade do número de observações em relação à área, considerando os níveis categóricos 0 e 1. Segue a seguinte fórmula:

$$
Log(N_i) = B_0 + B_1 * log(área_i) + \epsilon_i
$$

O resultado dos betas demonstra uma oferta inelástica de informação em relação à área:
```{r}
#| message: false
#| warning: false
...
O valor da terra por quantil ao longo de toda a amostra, lembrando que os valores aqui apresentados são fruto de correções inflacionárias de todos os valores para o ano de 2023.
```{r}
#| message: false
#| warning: false
...
Geograficamente, os dados refletem uma estrutura que respeita o esperado: os estados das regiões Sul, Sudeste e Centro-Oeste apresentam maior valor para a terra com imóvel, enquanto as regiões Norte e Nordeste apresentam valores inferiores. Uma anomalia observada é a região de Manaus, que compõe a Zona Homogênea de Terras “Manaus e Entorno”. Os valores dos imóveis provavelmente aumentam em razão da pressão da zona urbana, como ocorre tipicamente em áreas altamente urbanizadas, nas quais os imóveis tendem a apresentar maior valor.
```{r}
#| message: false
#| warning: false
library(dplyr)
...
A distribuição geográfica do VTN segue, em linhas gerais, a mesma lógica. É interessante notar a falta de dados acerca do valor da vegetação nativa.
...
A análise das linhas de densidade (Valor Total do Imóvel — VTI) demonstra um comportamento de multimodalidade para todas as categorias na distribuição dos preços, apresentando, de forma notável, uma quantidade mais acentuada de picos e vales nas nomenclaturas categóricas “pecuária”, “agricultura” e “vegetação nativa”.
```{r}
#| message: false
#| warning: false
library(plotly)
...
A análise das linhas de densidade do VTN (valor de terra nua) demonstra um comportamento de multimodalidade para todas as categorias na distribuição dos preços, apresentando o mesmo tipo de comportamento observado no VTI.
...
O preço do VTN apresenta, em geral, maior grau de variância em relação à média quando comparado ao VTI e, geralmente, valores máximos menores, provavelmente em razão das variações tecnológicas. Além disso, é possível observar que a tendência geral é que os valores de terras com imóvel sejam maiores do que os de terras sem imóvel.
...
INCRA. Superintendência Regional de São Paulo. Relatório de Análise de Mercados de Terras – São Paulo. Brasília, DF: Instituto Nacional de Colonização e Reforma Agrária, [2026?]. Disponível em: https://www.gov.br/incra/pt-br/assuntos/governanca-fundiaria/relatorio-de-analise-de-mercados-de-terras/sao-paulo. Acesso em: 15 dez. de 2025.
​