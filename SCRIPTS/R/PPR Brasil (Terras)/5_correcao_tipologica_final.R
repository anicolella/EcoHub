...
# LIMPEZA NUCLEAR: Remove tudo, mas poupa o dataframe e as variáveis do loop pai
rm(list = setdiff(ls(), c("df_joined", "mun_data" ,"script", "scripts", "caminho_base")), envir = .GlobalEnv)

df_joined <- st_as_sf(df_joined)
st_crs(df_joined) <- st_crs(mun_data)

# Criando o pipe com as áreas
df_joined <- df_joined %>%
  mutate(
    # Calcula a área em m² e converte para numérico puro
    area_m2 = as.numeric(st_area(geometry)),
    
    # Divide por 10.000 para converter m² para hectares
    area_ha_calculada = area_m2 / 10000
  )


df_joined_limpo <- df_joined %>%
  mutate(
...
df_total <- st_as_sf(df_classificado)  |> select(
    mrt,
    origem,
    code_muni,
    cluster,
...
df_ramt_completo <- st_as_sf(df_total)

st_crs(df_ramt_completo) <- st_crs(mun_data)

# Define o caminho relativo para a pasta data dentro do seu projeto
caminho_dados <- file.path(getwd(), "data")

# Se a pasta 'data' ainda não existir, o R cria ela para você na hora
if (!dir.exists(caminho_dados)) {
  dir.create(caminho_dados)
}

# Salva o arquivo dentro da pasta data usando caminho relativo
st_write(
  obj = df_ramt_completo, 
