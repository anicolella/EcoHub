...
dicionario_data <- tribble(
  ...
  "area_ha_calculada", "Espacial", "Área em hectares quadrados",4
...
Observação, as Zonas Homogêneas do Distrito Federal contemplam uma série de municípios que não são do estado de Goiás, nesse sentido, os documentos do Distrito apresentam também municípios de Minas Gerais e Bahia. As correções da geolocalização desses municípios pode ser vista na sessão de metodologia.
...
```{r}
# --- 0. Dependências e Setup ---
library(dplyr)
library(ggplot2)
library(sf)
library(scales)
library(stringr)
library(patchwork) # <--- ESSENCIAL

# Definição do Escopo Reduzido (Custo Te = 4)
# Removida 'vegetacao nativa' por ser tipicamente discrepante das demais em termos de VTI
categorias_alvo <- c("agricultura", "pecuaria", "media geral", "exploracao mista")
lista_plots <- list()

# --- 1. Execução Iterativa (Ciclo de Feedback Sunk Cost) ---
for(cat in categorias_alvo) {
  
  # A. Filtragem e Amostragem (Ponto Ótimo de Dados)
  dados_cat <- df_analise2 |>
    filter(ano < 2025, !is.na(IGPDI_vti_media), IGPDI_vti_media > 0) |>
    filter(categoria_final == cat) |> 
    group_by(code_muni) |>
  ...
  # B. Geração de Classes (Quantis)
  dados_cat <- dados_cat |>
    mutate(
      classe_raw = cut_number(IGPDI_vti_media, n = 5, dig.lab = 15),
      classe_preco = classe_raw 
    )

  # --- C. O TRATAMENTO DE STRING (Regex) - Mantido ---
  niveis_originais <- levels(dados_cat$classe_preco)
  niveis_limpos <- str_remove_all(niveis_originais, "[\\(\\]\\[\\)]")
  niveis_limpos <- str_replace(niveis_limpos, ",", " - ")
  
  # Aplicação de Labels (Qualitativo + Quantitativo Limpo)
  levels(dados_cat$classe_preco) <- paste0(
    c("Muito Baixo", "Baixo", "Médio", "Alto", "Muito Alto"), 
    "\n", 
    niveis_limpos 
  ...
  # D. Plotagem Estática (Otimizada para Composição)
  p <- ggplot(dados_cat) +
    geom_sf(aes(fill = classe_preco), color = NA, lwd = 0) +
    ...
    scale_fill_brewer(
      ...
      direction = -1,
      name = "Faixa de Preço (R$/ha)",
      guide = guide_legend(keyheight = unit(0.9, "cm")) # Melhora legibilidade do text \n
    ) +
    
    labs(
      title = paste(toupper(cat)), # Título limpo
      x = NULL, y = NULL
    ) +
    
    theme_void() +
    theme(
      plot.title = element_text(face = "bold", size = 12, hjust = 0.5), # Títulos menores e centralizados
      legend.position = "right",
      legend.text = element_text(size = 8)
    )

  lista_plots[[cat]] <- p
  message(paste("Mapa gerado:", cat))
}

# --- 2. COMPOSIÇÃO COM PATCHWORK (Arranjo Te = 2x2) ---

figura_composta <- (
  # Linha 1
  (lista_plots[["agricultura"]] | lista_plots[["pecuaria"]]) / 
  # Linha 2
  (lista_plots[["media geral"]] | lista_plots[["exploracao mista"]])
) + 
  
  # --- 3. ANOTAÇÕES E LAYOUT COLETIVO (Sinal > Ruído) ---
  plot_layout(
    guides = "collect" # <--- ESSENCIAL: Coleta e unifica a legenda à direita
  ) + 
  
  plot_annotation(
    title = "**Panorama do VTI no Agronegócio Brasileiro**",
    subtitle = "Distribuição de Preços Relativa por Categoria (Quantis)",
    caption = "Fonte: Dados de auditoria. Processamento próprio. Último ano disponível por município.",
    theme = theme(
      plot.title = element_text(face = "bold", size = 18, hjust = 0.5),
      plot.subtitle = element_text(size = 13, hjust = 0.5),
      plot.caption = element_text(size = 9, face = "italic", color = "grey40")
    )
  )

# Visualizar a figura final
figura_composta
```
