...
A otimização do procedimento de cluster é feito por meio do processo de "K-means". Esse algoritmo forma K grupos diversos de municípios que posteriormente serão modificados movendo cidades de um local ao outro de modo a maximizar ou minimizar a função, aumentando a qualidade do cluster( BRASIL,2014 ). As variáveis que são recomendadas para sereme levadas em conta durante o processo de formação agrupamentos (Zonas Homogêneas), pelas superintendências do INCRA, são as seguintes (BRASIL, 2014):

1 - Variáveis relacionadas à vocação produtiva da terra
- área total (hectares)

- área total de lavouras permanentes (% da área total)

 - área total de lavouras temporárias (% da área total)

 - área total de lavouras temporárias em descanso (pousio) (% da área total)

 - área total de pastagem natural (% da área total)
 - área total de pastagem plantada (% da área total)
 - área total de matas e florestas naturais (% da área total)
 - área total de matas e florestas plantadas (% da área total)
 - área total de terras não utilizáveis (% da área total)
...
Após esses processos, são formadas as zonas homogêneas ou os mercados de terra sob os quais são realizadas as análises do INCRA, com a finalidade de determinar os valores de terra na região (BRASIL, 2014). O principal componente das PPRs é a presença de um valor médio para Terra Nua e um valor médio para terra com imóvel, variando de acordo com a tipologia indicada. Essas médias são construídas mediante saneamento das amostras (método de ± 1 desvio padrão), sendo os novos saneamentos realizados até que o coeficiente de variação seja inferior ou igual a 30% (BRASIL, 2014).
...
O Fluxo de scripts é basicamente o mesmo, o que muda é que a correção inflacionária segue o seguinte script (este já contendo a função de recorte temporal):
...
dicionario_data <- tribble(
  ...
  "vtn_media", "Monetária", "Limite inferior da terra distribuição de terras sem benfeitorias (por hectare).",
...
Panorama Geográfico para o vTN:
```{r}
#| message: false
#| warning: false
...
dados_panorama <- dados_panorama |>
  ...
  mutate(
    # cut_number garante o mesmo número de observações em cada cor
    classe_preco = cut_number(vtn_media, n = 5, labels = c("Muito Baixo", "Baixo", "Médio", "Alto", "Muito Alto"))
  ) |>
  ungroup()
...
O VTN apresenta uma distribuição também multimodal; entretanto, diferencia-se da distribuição apresentada pelos diagramas do VTI. Essa divergência decorre, provavelmente, de diferenças estruturais na formação do preço: o VTI reflete, em alguma medida, a estrutura de capital já investido, enquanto o VTN reflete, preponderantemente, diferenças geográficas. Nesse sentido, é possível ver que a linha de desnsidaade é muito mais suave no gráfico do VTN que provavelmente não reflete tanto essas diferenças tecnológicas.
...
## 4.2 Análise Descritiva DataFrame 2023
